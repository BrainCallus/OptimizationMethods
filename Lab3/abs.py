...
class absNewton(ABC):
    ...
    def recoverCoefs(self, x: np.ndarray, y: np.ndarray, init_data: np.ndarray = None):
        ...
        epoch = 1
        iter = 0
        while np.abs(prev_square - cur_square) >= self.eps and iter <= self.max_iter:
            ...
            self.logger.info(f"Epoch {epoch}: CUR {cur_square}")
            delta = np.abs(prev_square - cur_square)
            if delta < self.eps:
                self.logger.info("Required error achieved")
                return epoch, iter
            if cur_square < self.min_val:
                self.logger.info("Min val for square reached.")
                return epoch, iter
            prev_square = cur_square
            epoch = epoch + 1
        self.logger.info("Max number of iterations reached. Regression didn't converge.")
