...
def draw_function_graph(func, grad, given_matrix=None, given_vector=None):
    ...
    ax.plot(xs1, ys1, 'o-', label = 'learning_rate: ' + str(points1[0]))
    ax.plot(xs2, ys2, 'o-', label = 'golden       : ' + str(points2[0]))
    ax.plot(xs3, ys3, 'o-', label = 'wolfe        : ' + str(points3[0]))
...
def multiple_tests_to_excel():
    ...
    ii = [2, 5, 10, 50, 100, 150, 200, 250, 300, 350, 400, 450,
        ...
        for j in jj:
            ...
            for k in range(times):
                ...
                points1, points2, points3 = \
                    analyze_function(dimensions, func, grad, vector_using=[True, True, True])
                average[0] += points1[0]
                average[1] += points2[0]
                average[2] += points3[0]
...
def functions_compare(func1, grad1, func2, grad2, method=1, func_name=None):
    ...
    given_vector = [-150, 150]
    al = 10

    p1 = analyze_function(2, func1, grad1, vector_using=vector_using, given_vector=given_vector)
    p2 = analyze_function(2, func2, grad2, vector_using=vector_using, given_vector=given_vector)
    points1 = p1[method - 1]
    points2 = p2[method - 1]
    ...
    x0 = np.linspace(left - al, right + al, 1000)
    y0 = np.linspace(bottom - al, up + al, 1000)
    ...
    ax.plot(xs1, ys1, 'o-', label = method_name + ': ' + str(points1[0]))
    ax.plot(xs2, ys2, 'o-', label = method_name + ': ' + str(points2[0]))
    ax.legend(prop='monospace')
...
def analyse_eps_parameter(func, grad):
    ...
    iterations, p1, p2 = method_mas(lr, start, 9, func, grad)
    ...
    for i in range(len(p1)):
        ...
        print('Итер: ' + str(iterations))
        print('Рез : ' + str(p2[i][-1]))

    iterations, p1, p2 = method_mas(golden, start, 9, func, grad)
